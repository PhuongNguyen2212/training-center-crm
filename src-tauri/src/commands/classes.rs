...
type ClassRow = (String, String, String, Option<String>, String, String, String);
...
async fn build_class(conn: &Connection, row: ClassRow) -> AppResult<Class> {
    ...
    Ok(Class { id, name, course_name, teacher_id, student_ids, status, created_at, updated_at })
}
...
async fn name_of(conn: &Connection, table: &str, id: &str) -> String {
    ...
    query_opt(conn, &sql, libsql::params![id.to_string()], |r| r.get::<String>(0))
        .await
        .ok()
        .flatten()
...
pub async fn list_classes_impl(token: &str, db: &Db, sessions: &Sessions) -> AppResult<Vec<Class>> {
    ...
    let rows = if user.role == "teacher" {
        query_all(&db.conn, &format!("{base} WHERE teacher_id = ?1"), libsql::params![user.id.clone()], map_class_row).await?
    } else {
...
pub async fn create_class_impl(token: &str, input: ClassInput, db: &Db, sessions: &Sessions) -> AppResult<Class> {
    let user = current_user(db, sessions, token).await?;
    ...
    db.conn
        .execute(
            ...
            libsql::params![id.clone(), input.name.trim().to_string(), input.course_name.trim().to_string(), input.teacher_id.clone(), now],
        )
        .await?;
    write_audit(&db.conn, &user.id, "class.create", &format!("Tạo lớp {}", input.name)).await?;
    one_class(&db.conn, &id).await
}

pub async fn update_class_impl(token: &str, id: String, input: ClassInput, db: &Db, sessions: &Sessions) -> AppResult<Class> {
    let user = current_user(db, sessions, token).await?;
    ...
    db.conn
        .execute(
            ...
            libsql::params![id.clone(), input.name.trim().to_string(), input.course_name.trim().to_string(), teacher_id, now_iso()],
        )
        .await?;
    write_audit(&db.conn, &user.id, "class.update", &format!("Cập nhật lớp {}", input.name)).await?;
    one_class(&db.conn, &id).await
}

pub async fn set_class_status_impl(token: &str, id: String, status: String, db: &Db, sessions: &Sessions) -> AppResult<()> {
    let user = current_user(db, sessions, token).await?;
    require_capability(&user, Capability::ScheduleEdit)?;
    ...
    db.conn
        ...
        .await?;
    write_audit(&db.conn, &user.id, "class.status_change", &format!("Lớp {id} → {status}")).await?;
    Ok(())
}

pub async fn enroll_student_impl(token: &str, class_id: String, student_id: String, db: &Db, sessions: &Sessions) -> AppResult<()> {
    let user = current_user(db, sessions, token).await?;
    require_capability(&user, Capability::ScheduleEdit)?;
    db.conn
        .execute(
    ...
    write_audit(&db.conn, &user.id, "class.enroll", &format!("Ghi danh {sname} vào lớp {cname}")).await?;
    Ok(())
}

pub async fn unenroll_student_impl(token: &str, class_id: String, student_id: String, db: &Db, sessions: &Sessions) -> AppResult<()> {
    let user = current_user(db, sessions, token).await?;
    require_capability(&user, Capability::ScheduleEdit)?;
    db.conn
        .execute(
    ...
    let sname = name_of(&db.conn, "students", &student_id).await;
    let cname = name_of(&db.conn, "classes", &class_id).await;
    write_audit(&db.conn, &user.id, "class.unenroll", &format!("Hủy ghi danh {sname} khỏi lớp {cname}")).await?;
    Ok(())
}
...
pub async fn delete_class_impl(token: &str, id: String, db: &Db, sessions: &Sessions) -> AppResult<()> {
    let user = current_user(db, sessions, token).await?;
    require_capability(&user, Capability::ScheduleEdit)?; // admin
    let cname = name_of(&db.conn, "classes", &id).await;
    db.conn
        .execute("DELETE FROM class_students WHERE class_id=?1", libsql::params![id.clone()])
        .await?;
    db.conn
        .execute("UPDATE sessions SET class_id=NULL WHERE class_id=?1", libsql::params![id.clone()])
        .await?;
    db.conn
        .execute("DELETE FROM classes WHERE id=?1", libsql::params![id.clone()])
        .await?;
    write_audit(&db.conn, &user.id, "class.delete", &format!("Xóa lớp {cname}")).await?;
    Ok(())
}
...
#[cfg(feature = "desktop")]
#[tauri::command]
pub async fn list_classes(token: String, db: State<'_, Db>, sessions: State<'_, Sessions>) -> AppResult<Vec<Class>> {
    list_classes_impl(&token, &db, &sessions).await
}

#[cfg(feature = "desktop")]
#[tauri::command]
pub async fn create_class(token: String, input: ClassInput, db: State<'_, Db>, sessions: State<'_, Sessions>) -> AppResult<Class> {
    create_class_impl(&token, input, &db, &sessions).await
}

#[cfg(feature = "desktop")]
#[tauri::command]
pub async fn update_class(token: String, id: String, input: ClassInput, db: State<'_, Db>, sessions: State<'_, Sessions>) -> AppResult<Class> {
    update_class_impl(&token, id, input, &db, &sessions).await
}

#[cfg(feature = "desktop")]
#[tauri::command]
pub async fn set_class_status(token: String, id: String, status: String, db: State<'_, Db>, sessions: State<'_, Sessions>) -> AppResult<()> {
    set_class_status_impl(&token, id, status, &db, &sessions).await
}

#[cfg(feature = "desktop")]
#[tauri::command]
pub async fn enroll_student(token: String, class_id: String, student_id: String, db: State<'_, Db>, sessions: State<'_, Sessions>) -> AppResult<()> {
    enroll_student_impl(&token, class_id, student_id, &db, &sessions).await
}

#[cfg(feature = "desktop")]
#[tauri::command]
pub async fn unenroll_student(token: String, class_id: String, student_id: String, db: State<'_, Db>, sessions: State<'_, Sessions>) -> AppResult<()> {
    unenroll_student_impl(&token, class_id, student_id, &db, &sessions).await
}

#[cfg(feature = "desktop")]
#[tauri::command]
pub async fn delete_class(token: String, id: String, db: State<'_, Db>, sessions: State<'_, Sessions>) -> AppResult<()> {
