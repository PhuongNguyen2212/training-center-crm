...
pub async fn create_staff_impl(
    token: &str,
    name: String,
    email: String,
    role: String,
    ...
    write_audit(
        ...
        &actor.id,
        "staff.create",
        &format!("Tạo tài khoản {name} ({role})"),
    )
    .await?;
    query_opt(
        &db.conn,
...
pub async fn set_user_status_impl(
    token: &str,
    id: String,
    status: String,
    db: &Db,
    ...
    write_audit(
        ...
        &actor.id,
        action,
        &format!("Đổi trạng thái tài khoản {id} → {status}"),
    )
    .await?;
    Ok(())
}

pub async fn update_user_role_impl(
    token: &str,
    id: String,
    role: String,
    db: &Db,
    ...
    db.conn
        .execute(
            ...
            libsql::params![id.clone(), role.clone(), now_iso()],
        )
        .await?;
    write_audit(
        &db.conn,
        &actor.id,
        "staff.role_change",
        &format!("Đổi vai trò {id} → {role}"),
    )
    .await?;
    Ok(())
}

pub async fn reset_user_password_impl(
    token: &str,
    id: String,
    password: String,
    db: &Db,
